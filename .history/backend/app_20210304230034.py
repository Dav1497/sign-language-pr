...
def getAllLevels():
    if request.method == 'GET':
        return LevelstHandler.getAllLevels()
    elif request.method == 'POST':
        return LevelstHandler.createLevel(request.json) 
    else:
        return jsonify(message="Method not allowed."), 405
...
def getLevelById(lid):
    if request.method == 'GET':
        return LevelstHandler.getLevelById(lid)
    elif request.method == 'PUT':
        return LevelstHandler.updateLevel(lid, request.json)
    elif request.method == 'DELETE':
        return LevelstHandler.deleteLevel(lid)
    else:
        return jsonify(message="Method not allowed."), 405
